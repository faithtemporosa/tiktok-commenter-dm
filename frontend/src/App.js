            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              <table className="w-full text-sm" data-testid="post-table">
                ...
                <tbody>{postReports.length===0?<tr><td colSpan="5" className="text-center py-12 text-zinc-500"><Video className="w-6 h-6 mx-auto mb-2 opacity-50" />No posts yet</td></tr>
                  :postReports.map((p,i)=>{const mappedUsername = p.tiktok_username || profileMappings[p.profile]; const repostLink = p.repost_url || (mappedUsername && `https://www.tiktok.com/@${mappedUsername}`); return <tr key={p.id||i} className="border-t border-zinc-800 hover:bg-zinc-800/30" data-testid={`post-row-${i}`}><td className="px-4 py-3 text-zinc-400 text-xs">{fmt(p.timestamp)}</td><td className="px-4 py-3"><span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-300 text-xs">{p.profile}</span></td><td className="px-4 py-3">{repostLink?<a href={repostLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-rose-400 hover:text-rose-300 text-xs"><ExternalLink className="w-3 h-3" />@{mappedUsername || 'View'}</a>:<span className="text-zinc-500 text-xs">Set in Settings</span>}</td><td className="px-4 py-3 text-zinc-300 text-xs max-w-md">{trunc(p.caption,60)}</td><td className="px-4 py-3"><span className="px-2 py-0.5 rounded text-xs text-emerald-400 bg-emerald-500/20">{p.status}</span></td></tr>})}</tbody>
              </table>
            </div>
          </div>
        )}
