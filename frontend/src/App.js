            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
              <table className="w-full text-sm" data-testid="post-table">
                ...
                <tbody>{postReports.length===0?<tr><td colSpan="5" className="text-center py-12 text-zinc-500"><Video className="w-6 h-6 mx-auto mb-2 opacity-50" />No posts yet</td></tr>
                  :postReports.map((p,i)=><tr key={p.id||i} className="border-t border-zinc-800 hover:bg-zinc-800/30" data-testid={`post-row-${i}`}><td className="px-4 py-3 text-zinc-400 text-xs">{fmt(p.timestamp)}</td><td className="px-4 py-3"><span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-300 text-xs">{p.profile}</span></td><td className="px-4 py-3">{p.video?<a href={p.video} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-violet-400 hover:text-violet-300 text-xs"><ExternalLink className="w-3 h-3" />View</a>:'-'}</td><td className="px-4 py-3 text-zinc-300 text-xs max-w-md">{trunc(p.caption,60)}</td><td className="px-4 py-3"><span className="px-2 py-0.5 rounded text-xs text-emerald-400 bg-emerald-500/20">{p.status}</span></td></tr>)}</tbody>
              </table>
            </div>
          </div>
        )}
